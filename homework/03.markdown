...
* Create a new web application
    <pre><code data-trim contenteditable>var App = require('./miniWeb.js');
var app = new App();
</code></pre>
...
Note that when you import the module, you can set a variable directly to it that serves as a constructor. This is done by `module.exports = App;` within the module itself.
...
3. Run your server and check all 3 of these paths using both curl and your browser.
...
4. Finally, to see how everything works together, modify the html that `/` sends back:
...
5. It should look something like <br> ![pic alt](../resources/img/hw03-02-evenWarmer-01-request.gif)
...
2. To send back a response, remove your calls to `sock.write` and use a new `Response` object instead.
3. Again, within the callback that you pass to `sock.on('data', someCallback)` and after you create a `Request` object, create a `Response` object....
...
4. Replace every call to `sock.write` with:
...
5. Everything should still work as it did previously, so try using curl and your browser to request `/`, `/foo.css`, and any other page to trigger a 404
...
4. Export your `App` constructor: `module.exports = App`
